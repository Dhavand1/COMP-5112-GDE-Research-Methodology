...
# Here, we will sort the contours from left to right
...
# Applying adaptive thresholding and extract Value channel from the HSV format of image
# This will reveal the characters on the license plate
...
def segment_chars(plate_img, fixed_width):
    ...
    # Resizing the license plate region to a canoncial size
    ...
    # Initializing the mask to store the location of the character candidates
    ...
    for label in np.unique(labels):
        ...
        # Constructing the label mask to display only connected components for the current label
        ...
        # Ensure that at least one contour is found in the mask
        
        if len(cnts) > 0:

            # Grab the largest contour which corresponds to the component in the mask
            # Also, grab the bounding box for the contour
            
            c = max(cnts, key=cv2.contourArea)
            (boxX, boxY, boxW, boxH) = cv2.boundingRect(c)

            # Computing the aspect ratio, solodity, and height ratio
            
            aspectRatio = boxW / float(boxH)
            solidity = cv2.contourArea(c) / float(boxW * boxH)
            heightRatio = boxH / float(plate_img.shape[0])

            # Determining if the aspect ratio, solidity, and height of the contour pass?
            
            keepAspectRatio = aspectRatio < 1.0
            keepSolidity = solidity > 0.15
            keepHeight = heightRatio > 0.5 and heightRatio < 0.95

            # Checkpoint! to see if the component passes all the tests
    ...
    if contours:
        ...
        addPixel = 4  # This is the value to be added to each dimension of the character
        for c in contours:
...
class NeuralNetwork:
    ...
    # Take an image and tranform it in tensor
